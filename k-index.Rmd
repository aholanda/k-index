...
L-000-000;Joe Doe;http//joedoe.edu;55;200101311200
```
...
about author's publications. The fourth column is the author's 
`h-index` and the fifth is the timestamp representing the date and 
time when the `h-index` was updated.

# Indices Calculation

The $h$ is the Hirsch index
proposed by Hirsch [-@hirsch2005]. It is obtained at Web of Science, so no further
procedure is needed. 
The $K$ index and was proposed by Kinouch _et al._ [-@kinouchi2018] and its 
calculation is presented in the [section on K-index][K-index].
...
that is assigned to variable `k.index.tsv.sep`.

The file name assigned to `nobel.laureates.file.name` contains the
authors (identifier) that already received the Nobel prize. This list 
is used to eliminate them from the process of authors ranking.


```{r}
k.index.tsv.sep <- "\t"
nobel.laureates.file.name <- "laureates_in.csv"
```
